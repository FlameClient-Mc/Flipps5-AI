...
ADAPTER_DIR = "flameflipps-lora"
MAX_HISTORY_TURNS = 10


def load_model(model_name, adapter_dir):
    print(f"[FlameFlipps] Loading model: {model_name}")
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype=torch.float32)
    if adapter_dir and os.path.isdir(adapter_dir):
        from peft import PeftModel

        print(f"[FlameFlipps] Loading fine-tuned adapter: {adapter_dir}")
...
def chat_loop(tokenizer, model, max_tokens, temperature):
    print()
    print("  FlameFlipps is ready. Type 'exit' or 'quit' to leave.")
    print("  " + "-" * 60)
    history = []
    while True:
        try:
        ...
        print(f"FlameFlipps> {reply}")
        print()


def main():
    parser = argparse.ArgumentParser(description="FlameFlipps — your local AI assistant")
